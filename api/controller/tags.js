...
exports.search = (req, res) => {
  const defineParameters = (req) => {
    let parameters = {};

    console.log(req.query);

    if (req.query.name) {
      parameters["name"] = { $regex: req.query.name, $options: "i" };
    }
...
exports.reset = (req, res) => {
  Tag.deleteMany({})
    .exec()
    .then(() => {
      addAll(industries).then(() =>
        res.status(200).json({
          status: "success",
          message:
            "the Tag collection was cleared, and reset with a preset list of data",
        })
      );
    })
    .catch((error) => {
